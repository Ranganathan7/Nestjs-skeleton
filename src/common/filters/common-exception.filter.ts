...
export class CommonExceptionFilter implements ExceptionFilter {
  ...
  constructor(@Inject(LOGGER) private readonly logger: Logger) { }
    ...
    const errorResponse: CommonApiResponse<ApiErrorResponse> = {
      statusCode: status,
      message: error.name ? (error.name === 'HttpException' ? exceptionResponse[CONSTANTS.MESSAGE] : error.name) : '',
      timestamp: new Date().toISOString(),
      requestId: exceptionResponse[CONSTANTS.REQUEST_ID] ? exceptionResponse[CONSTANTS.REQUEST_ID] : '',
      error: {
        // errorCode: status,
        description: exceptionResponse[`error`]?.description ? exceptionResponse[`error`]?.description : error.message ? ((exceptionResponse[CONSTANTS.MESSAGE] instanceof Array) ? exceptionResponse[CONSTANTS.MESSAGE].join(' | ') : error.message) : '',
